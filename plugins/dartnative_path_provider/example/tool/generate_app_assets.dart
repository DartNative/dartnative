...
//
// One-step asset generator for dart_native apps.
...
// The splash screen background color is set in pubspec.yaml under dart_native_splash:.
// ─────────────────────────────────────────────────────────────────────────────
...
void main(List<String> args) async {
  ...
  final splashResult = await Process.run(
    'dart',
    ['run', 'dart_native_splash:setup'],
    workingDirectory: Directory.current.path,
  );
  print(splashResult.stdout.toString().trim());
  if (splashResult.exitCode != 0) {
    print('WARNING: dart_native_splash exit code ${splashResult.exitCode}');
